...
<!--% Remplacez ___ par votre prénom et votre nom dans l'entête YAML. -->
...
<!--% Complétez les éléments ___ dans le chunk ci-dessous afin d'importer et d'ajouter des étiquettes et des unités aux variables du tableau de données assigné à `achatina`. Les métadonnées se trouvent dans le README. -->

```{r import, message=FALSE, record='RODFS', object='achatina'}
read("___") %>.%
...
<!--% Faites un résumé de votre modèle avec la fonction `summary_()` appliquée sur votre objet `achatina_lm` et formatez le tableau avec `tabularise()`. -->

```{r linsummary, record='RNP', arg='col_keys', warning=FALSE}
___
```
...
```{r linresid, record='RNCS', arg='no.data'}
___
```
...
[] -   Les résidus ont une variance qui augmente inversement avec les valeurs prédites (hétéroscédasticité importante, C).
...
<!--% Faites un résumé de votre modèle avec la fonction `summary_()` appliquée sur votre objet `achatina_poly` et formatez-le avec `tabularise()`. -->
...
```{r polyresid, record='RNCS', arg='no.data'}
___
```
...
[] -   L'ANOVA comparant modèle linéaire simple et polynomial (modèles imbriqués) indique que le modèle le plus simple (la droite) est à préférer au seuil $\alpha$ de 5%.
[] -   L'ANOVA comparant modèle linéaire simple et polynomial (modèles imbriqués) indique que le modèle le plus complexe (polynomial) est à préférer au seuil $\alpha$ de 5%.
[] -   L'ANOVA comparant modèle linéaire simple et polynomial (modèles imbriqués) indique que les deux modèles ne sont pas significativement différents l'un de l'autre au seuil $\alpha$ de 5%.

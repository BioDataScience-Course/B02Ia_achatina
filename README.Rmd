...
**À la fin de votre étude, vérifiez que votre document `achatina_notebook.qmd` compile en un fichier final HTML sans erreurs via le bouton Knit.** Vous avez une batterie de tests à votre disposition ensuite pour vérifier que le travail est réalisé correctement (onglet "Construire" -> bouton "Construire tout"). Vérifiez enfin que vous avez **enregistré vos modifications**, puis faites un **commit**, un **pull** et un **push**. Vous pouvez directement vérifier dans le dépôt GitHub qu'il présente votre travail tel qu'il sera visible à la correction. **L'oubli d'un enregistrement et/ou d'un commit ou d'un push peut faire qu'une partie de votre travail ne sera pas pris en compte**, donc vérifiez plutôt deux fois qu'une.
...
Les données de ce projet ont été employées dans le cadre d'une publication de Van Osselaer et Grosjean [-@VanOsselaer2000]. Cette publication est disponible dans le dossier `biblio`. Évitez cependant de perdre trop de temps à lire la bibliographie et les formatages Markdown dans le cadre de cet exercice (vous pourrez toujours y revenir plus tard). Nous vous conseillons d'organiser votre temps comme suit :
